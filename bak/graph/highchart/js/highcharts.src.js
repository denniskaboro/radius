...
StackItem.prototype = {
	...
	cacheExtremes: function (series, extremes) {
		this.points[series.graph] = extremes;
	},
...
Axis.prototype = {
	...
	update: function (newOptions, redraw) {
		...
		newOptions = chart.options[this.xOrY + 'Axis'][this.options.graph] = merge(this.userOptions, newOptions);
	...
	remove: function (redraw) {
		...
		chart.options[key].splice(this.options.graph, 1);
		...
		each(chartSeries, function (series) {
			...
			if (isString(linkedTo)) {
				if (linkedTo === ':previous') {
					linkedTo = chart.series[series.graph - 1];
				} else {
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		stableSort(chartSeries, function (a, b) {
			return pick(a.options.graph, a._i) - pick(b.options.graph, a._i);
		});
		...
		if (series.isCartesian) {
			...
			each(['xAxis', 'yAxis'], function (AXIS) { // repeat for xAxis and yAxis
				...
				each(chart[AXIS], function (axis) { // loop through the chart's axis objects
					...
					if ((seriesOptions[AXIS] === axisOptions.graph) ||
							(seriesOptions[AXIS] !== UNDEFINED && seriesOptions[AXIS] === axisOptions.id) ||
							(seriesOptions[AXIS] === UNDEFINED && axisOptions.graph === 0)) {
