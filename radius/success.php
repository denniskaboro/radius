...
$r = mysqli_query($con, $in) or $err = mysqli_error($con);


$newId = $con->insert_id;
$newDate = date("d M Y H:i", strtotime("+1 month"));
$sql = "SELECT clients.mobile,groups.*, radcheck.value 
FROM radusergroup 
    INNER JOIN groups ON radusergroup.groupname=groups.id 
    INNER JOIN radcheck ON radusergroup.username=radcheck.username 
    INNER JOIN clients ON radusergroup.username=clients.username 
WHERE radusergroup.username='$BillRefNumber' and radcheck.attribute='Cleartext-Password'";
